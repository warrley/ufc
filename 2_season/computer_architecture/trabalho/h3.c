...
// uint16_t *adrs = (uint16_t *) malloc(n * sizeof(uint16_t));
int n = 0;
...
// #define SIMM10(i)   ((int32_t)((i) >> 4) & 0x3FF)
// #define SIMM12(i)   ((int32_t)((i) >> 4) & 0xFFF)

#define SIMM8(i)  ((int8_t)(((i) >> 4) & 0xFF))

int16_t extend12(uint16_t instr) { // -> 
    // 1. Isola os 12 bits superiores (15-4)
    // 2. Converte para int16_t para habilitar deslocamento aritmético
    // 3. O bit 11 do imediato (bit 15 da instrução) já está no topo.
    // 4. Desloca 4 bits para a direita para alinhar o valor, 
    //    mantendo o sinal (preenchimento com 1s ou 0s).
    return ((int16_t)instr) >> 4;
}
